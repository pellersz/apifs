...
fn parse_notification(args: &Vec<String>) -> Result<Box<dyn Command>, Error> {
    ...
    match args[2].as_str() {
        "once" => {
            ...
            if let Some(act_datetime) = final_datetime {
                return Ok(Box::new(Remind::new(Reminder::Once(
                    act_datetime,
                    Media { picture, sound },
                    description,
                ))));
        ...
        "daily" => {
            ...
            if let Some(act_time) = final_time {
                ...
                return Ok(Box::new(Remind::new(Reminder::Daily(
                    act_time,
                    days,
                    last_day,
                    Media { picture, sound },
        ...
        "interval" => {
            ...
            if let Some(act_datetime) = final_datetime {
                return Ok(Box::new(Remind::new(Reminder::SpecificInterval(
                    act_datetime,
                    interval_time,
                    Media { picture, sound },
                    description,
...
fn parse_note(args: &Vec<String>) -> Result<Box<dyn Command>, Error> {
    ...
    if let (Some(act_description), Some(act_name)) = (description, name) {
        return Ok(Box::new(WriteNote::new(Note {
            name: act_name,
...
fn parse_show(args: &Vec<String>) -> Result<Box<dyn Command>, Error> {
    let res = parse_select(args);
    match res {
        Ok(act_res) => Ok(Box::new(Show::new(act_res))),
        Err(err) => Err(err),
    }
}

fn parse_delete(args: &Vec<String>) -> Result<Box<dyn Command>, Error> {
    let res = parse_select(args);
    match res {
        Ok(act_res) => {
            ...
            Ok(Box::new(Delete::new(act_res)))
        }
        Err(err) => Err(err),
    }
}

pub fn parse_command(args: Vec<String>) -> Result<Box<dyn Command>, Error> {
...
// These tests no longer cut it, because now command is not an enum
//#[test]
//fn test_start() {
//    let Ok(res) = parse_command(vec![String::new(), String::from("start")]) else {
//        panic!("error");
//    };
//    match res {
//        Command::Start => {}
...
//#[test]
//fn test_stop() {
//    let Ok(res) = parse_command(vec![String::new(), String::from("stop")]) else {
//        panic!("error");
//    };
//    match res {
//        Command::Stop => {}
...
//#[test]
//fn test_help() {
//    let Ok(res) = parse_command(vec![String::new(), String::from("help")]) else {
//        panic!("error");
//    };
//    match res {
//        Command::Help => {}
